...
import ExploreProduct from './ExploreOurProducts'; // Import the product array

export function ExploreOurProduct() {
  return (
        ...
        {ExploreProduct.map((product) => (
          <div className="col-6 col-md-3" key={product.id}>
            <div className="border rounded p-3 position-relative h-100 product-card">
              ...
              {/* Make card clickable */}
              <Link to={`/ExploreProduct/${product.id}`} className="stretched-link"></Link>
            </div>
          </div>
        ))}
      </div>
